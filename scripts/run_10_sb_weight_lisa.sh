. ./activate.sh

experiment_name="m1m2_10_sb_weight"
sample_path="/project/druhe/gw/sampleDict_FAR_1_in_1_yr.pickle"
injection_path="/project/gw/injectionDict_FAR_1_in_1.pickle"
for sb_weight in 1 0.1 0.01 0.001 0.0001 0.00001 0.000001 0.0000001 0.00000001 0.000000001; do
    python_command="WANDB_ENABLED=TRUE bash main.sh configs/m1m2_sb_planar.yml --train \
        --trainer.min_epochs=128 \
        --model.sb_weight=$sb_weight \
        --dataset.sample_path=$sample_path \
        --dataset.injection_path=$injection_path \
        --experiment.name=$experiment_name"_"$sb_weight \
        --experiment.group=$experiment_name"
    launch_cmd="sbatch -p shared -t 24:00:00 --wrap '$python_cmd'"
    echo $launch_cmd
done

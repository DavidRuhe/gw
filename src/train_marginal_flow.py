...
def main(config):
    ...
    flow_object = object_from_config(config, "flow")
    flows = [
        flow_object(**config["flow"])
        for _ in range(train_dataset.dimensionality)
    ]
    ...
    if ckpt is not None:
        model = model_object.load_from_checkpoint(
            ckpt, flows=flows, d=train_dataset.dimensionality
        )
    else:
        model = model_object(

...
def process_gw_data(path):
    ...
    for n, event in events.items():
        ...
        q = m1 / torch.from_numpy(event["m2"])
        q = q.clamp(*Q_RNG)
        qmin = min(qmin, q.min())
        qmax = max(qmax, q.max())
        ...
        gw_data = torch.stack([m1, q, z, chi, z_prior, chi_prior], dim=-1).float()

        datasets.append(torch.utils.data.TensorDataset(gw_data))

    return ConcatDataset(*datasets), (m1min, m1max), (qmin, qmax), (zmin, zmax)


class M1QZChiDataset:
    dimensionality = 4
    has_normalization = True

    # n_grid = 32
    grid_m1 = np.linspace(*M_RNG, 32)
    grid_q = np.linspace(*Q_RNG, 31)
    grid_z = np.linspace(*Z_RNG, 30)
    grid_chi = np.linspace(*CHI_RNG, 29)
    grid = {
        "m1": grid_m1,
        "q": grid_q,
        "z": grid_z,
    ...
    def __init__(
        ...
        self.gw_data, self.m1minmax, self.qminmax, self.zminmax = process_gw_data(
            gw_path
        )
...
class M1QZChiSBDataset(M1QZChiDataset):
